<x-app-layout>
    ...
    <div class="container px-4 pt-6 mx-auto lg:px-8 lg:pt-8 xl:max-w-7xl">
        <div class="flex flex-col gap-2 text-center sm:flex-row sm:items-center sm:justify-between sm:text-start">
            <div class="grow">
                <h1 class="mb-1 text-xl font-bold">Create New Court</h1>
            </div>
        </div>
    </div>
    <!-- END Page Heading -->
                        ...
                        <form action="{{ route('admin.courts.store') }}" method="post" enctype="multipart/form-data">
                            ...
                            <div class="grid grid-cols-3 mb-6 gap-x-3 gap-y-4">
                                ...
                                <div>
                                    ...
                                    <x-text-input id="weekday_price" name="weekday_price" type="number"
                                        class="block w-full mt-1" />
                                    <x-input-error :messages="$errors->get('weekday_price')" class="mt-2" />
                                </div>
                                <div>
                                    <x-input-label for="weekend_price" :value="__('Weekend Price')" />
                                    <x-text-input id="weekend_price" name="weekend_price" type="number"
                                        class="block w-full mt-1" />
                            ...
                            <x-primary-button>{{ __('Create New Courter') }}</x-primary-button>
                        </form>
                    </div>
